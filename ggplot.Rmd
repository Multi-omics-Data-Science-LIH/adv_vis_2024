...
LGG %>%
  filter(!is.na(Sex)) %>%
ggplot()+
  geom_point(aes(x = Diagnosis.Age, y=MSI.MANTIS.Score), color="red")
...
LGG %>%
  ...
  ggplot() +
  geom_bar(aes(x=categories,  fill=Race.Category), color="black")
...
LGG %>%
  ...
  ggplot(LGG) +
  geom_bar(aes(x=Sex,  fill=Race.Category))
...
LGG %>%
  filter(!is.na(Subtype) & !is.na(Sex)) %>%
 ggplot(aes(x = Subtype, y=MSI.MANTIS.Score, fill=Sex, color=Sex)) + geom_boxplot( width=0.4) + geom_jitter(width = 0.08, alpha=0.7)
...
ggplot(LGG,aes(x = Diagnosis.Age, y=MSI.MANTIS.Score))+
  ...
  scale_x_continuous(name="Age of diagnosis", breaks = c(0,10,50,60,100), labels = c("zero",10,50,60,100), limits = c(0,100), position = "top")
...
ggplot(LGG,aes(x = Diagnosis.Age, y=MSI.MANTIS.Score))+
  geom_point(aes(color=Sex))+
  stat_smooth(method = "lm", formula = y ~ x, se=T, level=0.95, color="red") + 
...
```{r}
p + theme_minimal() +
  theme( legend.position = "none" , text=element_text(angle = 45, color = "turquoise"))
...
```{r plots, fig.format=c("pdf", "png"), fig.width=5, fig.height=6}
...
```{r}
LGG.sum %>% 
  mutate(Cancer.Type.Detailed = fct_reorder(Cancer.Type.Detailed, desc(Aneuploidy.Score))) %>%
ggplot( aes(x=Cancer.Type.Detailed, y=Aneuploidy.Score)) + 
  geom_bar(stat="identity")
